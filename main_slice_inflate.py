...
def get_model_input(batch, config, num_classes, sa_atm, hla_atm, sa_cut_module, hla_cut_module):
    ...
    # b_input = torch.stack([sa_label_slc, sa_label_slc], dim=1).argmax(2)
    # b_input = b_input.view(-1, 2, W_TARGET_LEN, W_TARGET_LEN) # TODO change this and input hla_slc again

    b_input = torch.cat([sa_label_slc, hla_label_slc], dim=1)
    ...
    for batch_idx, batch in tqdm(enumerate(dataloader), desc=phase, total=len(dataloader)):
        if phase == 'train':
            ...
            if epx % 10 == 0 and '1010-mr' in batch['id']:
                ...
                # nib.save(nib.Nifti1Image(b_input[idx].split(0).int().detach().cpu().numpy(), affine=np.eye(4)), _dir.joinpath(f"input_epx_{epx}.nii.gz"))
                nib.save(nib.Nifti1Image(torch.stack([slc.argmax(0) for slc in b_input[idx].int().detach().cpu().chunk(2)]).int().detach().cpu().numpy(), affine=np.eye(4)), _dir.joinpath(f"input_epx_{epx}.nii.gz"))
