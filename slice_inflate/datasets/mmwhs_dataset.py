...
from slice_inflate.datasets.align_mmwhs import nifti_transform, crop_around_label_center, cut_slice
...
def load_data(self_attributes: dict):
    ...
    for _3d_id, _file in tqdm(id_paths_to_load, desc=description):
        ...
        additional_data_3d[_3d_id] = dict(
            nifti_affine=torch.from_numpy(nib_tmp.affine).float(),
            align_affine=torch.from_numpy(np.loadtxt(align_affine_path)).float()
        )
